...
%%% 
%%% See `hb_ao' for more information about the Converge Protocol
...
set(Msg, InputPath, Value, Opts) ->
    ...
    NewPriv = hb_ao:set(Priv, Path, Value, priv_converge_opts(Opts)),
...
set(Msg, PrivMap, Opts) ->
    ...
    NewPriv = hb_ao:set(CurrentPriv, PrivMap, priv_converge_opts(Opts)),
    ?event({set_private_res, {out, NewPriv}}),
    set_priv(Msg, NewPriv).
