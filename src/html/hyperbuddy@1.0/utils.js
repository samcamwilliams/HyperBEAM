...
class SimpleJsonViewer {
    ...
    createNode(value, key, level) {
        ...
        if (valueType === 'object' || valueType === 'array') {
            ...
            const count = valueType === 'object' ? Object.keys(value).length : value.length;
            ...
            container.className = `json-container ${this.options.expand ? '' : 'collapsed'}`;
            ...
            // For arrays
                ...
                value.forEach((arrItem, index) => {
                    const childNode = this.createNode(arrItem, index.toString(), level + 1);
                    container.appendChild(childNode);
                });
            }
    ...
    expandAll() {
        ...
        const containers = this.options.container.querySelectorAll('.json-container');
