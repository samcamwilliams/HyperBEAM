...
-define(TPM2_CREATE_CMD, "tpm2_create -C " ++ ?PRIMARY_CTX_FILE ++ " -G rsa -u " ++ ?AK_PUB_FILE ++ " -r " ++ ?AK_PRIV_FILE).
-define(TPM2_LOAD_CMD, "tpm2_load -C " ++ ?PRIMARY_CTX_FILE ++ " -u " ++ ?AK_PUB_FILE ++ " -r " ++ ?AK_PRIV_FILE ++ " -c " ++ ?AK_CTX_FILE).
-define(TPM2_READPUBLIC_CMD, "tpm2_readpublic -c " ++ ?AK_CTX_FILE ++ " -o " ++ ?AK_PUB_FILE ++ " -f pem").
-define(TPM2_QUOTE_CMD, "tpm2_quote -Q --key-context " ++ ?AK_CTX_FILE ++ " -l sha256:0,1 --message " ++ ?QUOTE_MSG_FILE ++ " --signature " ++ ?QUOTE_SIG_FILE ++ " --qualification ").
-define(TPM2_VERIFY_CMD, "tpm2_verifysignature -c " ++ ?AK_CTX_FILE ++ " -g sha256 -m " ++ ?QUOTE_MSG_FILE ++ " -s " ++ ?QUOTE_SIG_FILE).
...
generate_attestation(Nonce) ->
    ...
    case sec_helpers:run_command(Command) of
        {ok, _} -> 
            ...
            ?c({"Attestation generated, binary data:", AttestationBinary}),

            % Return the binary containing the header and files
            {ok, AttestationBinary};
        {error, Reason} -> 
            ?c({"Error generating attestation", Reason}),
            {error, Reason}
    end.
...
create_primary_key() ->
    ...
    case sec_helpers:run_command(CommandPrimary) of
        {ok, _} -> 
            ?c("Primary key created successfully"),
            create_attestation_key();
        {error, Reason} -> 
            ?c({"Error creating primary key", Reason}),
            {error, Reason}
    end.
...
create_attestation_key() ->
    ...
    case sec_helpers:run_command(CommandCreateAK) of
        {ok, _} -> 
            ?c("Attestation key created successfully"),
            load_attestation_key();
        {error, Reason} -> 
            ?c({"Error creating attestation key", Reason}),
            {error, Reason}
    end.
...
load_attestation_key() ->
    ...
    case sec_helpers:run_command(CommandLoadAK) of
        {ok, _} -> 
            ?c("Attestation key loaded successfully"),
            export_ak_public_key();
        {error, Reason} -> 
            ?c({"Error loading attestation key", Reason}),
            {error, Reason}
    end.
...
export_ak_public_key() ->
    ...
    case sec_helpers:run_command(Command) of
        {ok, _} -> 
            ...
            ok;
        {error, Reason} -> 
            ?c({"Error exporting AK public key", Reason}),
            {error, Reason}
    end.
