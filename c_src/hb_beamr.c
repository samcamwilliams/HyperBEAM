...
static int handle_wasm_function_call(wasm_exec_env_t exec_env, int function_index, uint64_t argc, uint64_t* argv) {
...
static int async_indirect_call(Proc* proc) {
    DRV_DEBUG("=================================================");
    ...
    DRV_DEBUG("Function index retrieved from input_args: %d", proc->current_function_ix);

    // Get the function reference from the table and cast it to a function
    wasm_ref_t* function_ref = wasm_table_get(proc->indirect_func_table, proc->current_function_ix);
    ...
    if (!function_type) {
        DRV_DEBUG("Failed to retrieve function type for function at index %d", proc->current_function_ix);
        return -1;
    }

    // Log the function's parameter types
    const wasm_valtype_vec_t* param_types = wasm_functype_params(function_type);
    DRV_DEBUG("Function at index %d has %zu parameters", proc->current_function_ix, param_types->size);
    ...
    DRV_DEBUG("Function at index %d has %zu results", proc->current_function_ix, result_types->size);
    ...
    prepare_function_arguments(&args, &prepared_args);
    ...
    for (uint64_t i = 0; i < argc; ++i) {
        argv[i] = args.data[i].of.i64;
    }

    result = handle_wasm_function_call(exec_env, proc->current_function_ix, argc, argv);
...
wasm_trap_t* generic_import_handler(void* env, const wasm_val_vec_t* args, wasm_val_vec_t* results) {
    ...
    // invoke_exported_function(proc, import_hook->field_name, args, results); // TODO: Add this line when using AOT
...
static void async_init(void* raw) {
    ...
    for (size_t i = 0; i < exports.size; i++) {
        ...
        if (is_indirect_function_table(name->data)) {
            ...
            proc->indirect_func_table = wasm_extern_as_table(exports.data[i]);
        }
