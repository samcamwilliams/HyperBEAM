...
static int invoke_exported_function(Proc* proc, const char *field_name, const wasm_val_vec_t* input_args, wasm_val_vec_t* output_results) {
    ...
    for (int i = 0; i < export_types.size; i++) {
        ...
        if (is_indirect_function_table(export_name->data)) {
            ...
            result = handle_wasm_function_call(exec_env, function_index, argc, argv);
...
static void async_init(void* raw) {
    ...
    wasm_runtime_set_default_running_mode(Mode_Interp); // TODO: Remove this line when using AOT
